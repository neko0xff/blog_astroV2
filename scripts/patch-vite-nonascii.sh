...
if [ ! -d "$VITE_DENO_DIR" ]; then
  echo "[Warn]  node_modules/.deno not found. Run 'deno install' first."
  exit 1
fi
...
if [ ! -f "$VITE_CONFIG" ]; then
  ...
  echo "[Info]  No vite@7.3.3 config.js found (installed: ${INSTALLED_VITE:-none})."
...
if grep -q 'let fromUrl;' "$VITE_CONFIG" 2>/dev/null; then
  echo "[Log] Vite non-ASCII path patch already applied."
  exit 0
fi

# Check that the original pattern exists
if ! grep -q 'const req\$4 = createRequire\$1(from);' "$VITE_CONFIG"; then
  echo "[Warn]  Unexpected Vite version - patch pattern not found."
...
if grep -q 'let fromUrl;' "$VITE_CONFIG"; then
  echo "[Log] Vite non-ASCII path patch applied successfully."
else
