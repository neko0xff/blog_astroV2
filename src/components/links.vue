...
const loadData = () => {
  var json_file = "/assets/myLinks.json";
  axios
    .get(json_file)
    .then(
      (resp: {
        data: { name: string; site: string; siteURL: string; icon: string }[];
      }) => {
...
<template>
  <div>
    <p class="text-xs text-right">
      📧:
      <a class="link" href="mailto:chzang55@gmail.com">chzang55@gmail.com</a>
    </p>
    <br />
    <div v-if="isLoading">
    ...
    <div v-else class="grid grid-cols-1 gap-3.5 md:grid-cols-2 mb-3">
      <div v-for="(card, index) in links" class="card-hover">
        <a :href="card.siteURL">
          <div
            class="relative overflow-hidden border-b-2 border-r-2 border-main avatar_img"
          >
            <img
              v-if="card.icon === ''"
