...
const CONSOLE_STYLES = {
  ...
  body:
    "font-size: 18px; line-height: 1.5; font-family: sans-serif; color: #333;",
  warning:
...
function showWarning(config) {
  ...
  const message1 = config.message1 ||
    "[警告] 此功能僅供開發者除錯使用，一般使用者請勿使用。";
  const message2 = config.message2 ||
    "[警告] 請勿在此貼上來源不明的程式碼和非官方指示。";
  const warning_msg = config.warning_msg ||
    "[警告] 這可能導致您的資料部分，直接被盜取。";
